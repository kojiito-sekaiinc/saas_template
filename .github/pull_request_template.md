...
## 🧭 Claude Workflow Check (Explore → Plan → Implement)

以下を実施しましたか？（非 trivial な変更の場合は必須）

- [ ] この変更は **Explore → Plan → Implement → Commit** ワークフローを使用した
- [ ] `/plan` モードでコードベースの探索のみを行い、変更は加えていない
- [ ] 実装前に、変更ファイル・手順・リスク・テストを含む **明示的な計画**を作成した
- [ ] 計画に基づいて実装し、途中でのスコープ逸脱はしていない
- [ ] 計画に含めたテスト／コマンドをすべて実行し、green を確認した

> ℹ️ タイポ修正や単純なリネームなど、  
> 明らかに機械的な変更の場合のみこのチェックは省略可。
...
- [ ] 非 trivial な変更について、docs/Workflows.md の Workflow 3 に従って進めた
