...
このテンプレートは以下の方を対象としています:

- 個人開発で SaaS を作りたいエンジニア
- Stripe 課金を最短で実装したい方
- Django で本番運用可能な構成をすぐに使いたい方
...
多くのテンプレートは「動く」ことにフォーカスしていますが、
このテンプレートは以下を重視しています:

- 課金の整合性（Webhook を正本とする設計）
- 障害時の復旧（sync コマンド）
- 本番事故の防止（check_deploy_config）
- CI による品質保証

「壊れない SaaS」を作るためのテンプレートです。
